...
curl -X POST 'http://192.168.1.26:8080/admin/config.php' \
...
cookie="$(curl -X POST -v --silent http://192.168.1.26:8080/admin/config.php \
...
curl -X POST 'http://192.168.1.26:8080/admin/ajax.php?module=api&command=add_application' \
    --header 'Referer: http://192.168.1.26:8080/admin/config.php?display=api' \
...
http://192.168.1.26:8080/admin/api/api/token >> /var/www/html/tokenAPI.json
